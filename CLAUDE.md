...
src/
  ...
  pages/
    ...
    admin/
      ...
      AdminMensalistas.jsx      ← **Página nova** em `/admin/mensalistas` (2026-08-10, ver seção "Estado atual") — gestão de mensalistas separada por mês, mesmo padrão de `AdminComissoes.jsx` abaixo. Toggle "Em aberto"/"Histórico de quitações"; KPIs (total em aberto/deste mês/atrasado/clientes pendentes) vêm de `GET /transaction/fiado-pending`'s `totals`; filtro de período (Todos/Mês atual/Mês passado/Escolher mês) + busca por cliente, ambos client-side. "Quitar" abre `ModalPagarMensalidade` (componente compartilhado, ver `components/ui/`).
...
| `/admin/mensalistas` | AdminMensalistas | Admin |
...
- **Toggle "Ocultar valores" movido para aba Configurações em Meu Perfil (2026-09-15, ⬜ não commitado, frontend puro)** — replicado do fork `Dauth-angelica` (Projeto Dauth Frontend ANG), onde esse padrão já tinha sido aplicado antes; pedido do usuário para trazer a mesma organização pra cá. `MeuPerfil.jsx` ganhou 2 abas (`TABS` — "Perfil"/"Configurações", state `tab`), mesmo padrão visual de pill sublinhada usado noutras telas do projeto; todos os blocos existentes (header "Editar", avatar card, view mode, histórico de agendamentos do cliente, edit mode) passaram a exigir `tab === 'perfil'` além das condições que já tinham. Aba "Configurações" mostra só o toggle "Ocultar valores financeiros" (`ToggleSwitch`, componente novo local — botão pill com bolinha deslizante, mesmo visual do resto do design system) ligado ao `usePrivacyStore` que já existia (ver "Feature — Ocultar valores" abaixo). **O botão de olho saiu do `Sidebar.jsx` (desktop) e do `AppLayout.jsx` (header mobile)** — antes era global/instantâneo em qualquer tela, agora só se alterna dentro de Meu Perfil; decisão confirmada explicitamente com o usuário via pergunta direta (opção "mover, igual ANG" vs. "duplicar, manter os dois") antes de implementar, para não remover acesso rápido sem aval. Nenhuma mudança de backend, de `privacyStore.js` ou de `MoneyValue.jsx` — só onde o toggle mora na UI. `npx vite build` limpo (1162 módulos). **Não testado na tela** — só build, sem abrir no navegador nesta sessão.
- **Feature — Ocultar valores (2026-09-14, ⬜ não commitado, frontend puro)** — pedido do usuário: poder borrar todos os valores monetários da tela na hora de mostrar o sistema para alguém (ex: cliente olhando por cima do ombro no balcão). `src/store/privacyStore.js` (Zustand, sem middleware — persiste manualmente em `localStorage['dauth_hide_values']`, mesmo padrão simples dos outros stores do projeto) expõe `{ hidden, toggle }`. `src/components/ui/MoneyValue.jsx` — componente que envolve um valor já formatado (`<MoneyValue>{formatCurrency(x)}</MoneyValue>`) e aplica a classe `.privacy-mask` (`filter: blur(5px)`, `index.css`) quando `hidden` está ativo; reage sozinho via hook, sem precisar de prop dos pais. Botão de olho (ícones `eye`/`eyeOff`, já existentes) adicionado em `Sidebar.jsx` (desktop, ao lado do sino) e `AppLayout.jsx` (header mobile) — global para Admin e Profissional, alterna instantaneamente. **Cobertura:** todo valor monetário renderizado nas telas autenticadas foi envolvido em `MoneyValue` — Dashboard (KPIs de receita/ticket médio, ranking de comissão, tooltip do gráfico), Caixa (comandas, painel de pagamento, produtos, relatório), Comissões (Admin e Profissional, cards, tabelas, modal de pagamento em lote, histórico), Mensalistas (KPIs, itens pendentes, histórico), Usuários (KPIs do cliente, crédito, comandas em aberto), Combos/Serviços/Produtos/Pedidos (Admin e Profissional, preços de catálogo), Comandas do cliente e Meus Combos, e os componentes compartilhados de crédito/troco (`CreditToggleRow`, `AmountTenderedField`, `CreditAndTrocoFields`, `ModalFecharConta`, `ModalPagarMensalidade`). **Não coberto, por decisão consciente:** valores dentro de `<option>` de `<select>` (browser não renderiza markup filho, só texto — 4 pontos: dropdown de produto em `AdminCaixa`/`ProfissionalComandas`/`AdminPedidosProdutos`/`ProfissionalPedidosProdutos`), texto de `placeholder` de input (mesma limitação de atributo puro), mensagens de toast/`addToast` (string plana, não JSX) e `AgendarPage.jsx` (fluxo público de autoagendamento do cliente — mostrar o próprio preço do serviço escolhido é o propósito da tela, não uma informação a esconder de terceiros). `npx vite build` limpo (1162 módulos) a cada etapa. **Não testado na tela** — só build, sem abrir no navegador nesta sessão.
- **Fix — data de agendamento 1 dia atrasada em Caixa/Comandas (2026-09-12, ⬜ não commitado, frontend puro)** — reportado pelo usuário: um agendamento de 11/09/2026 aparecia como 11/09 na Agenda, mas como 10/09 na tela de Caixa (`TabComandas`) e em `ProfissionalComandas`. **Investigação inicial equivocada:** o usuário também suspeitou que o agendamento (10 minutos de duração, 11:00–11:10) simplesmente não aparecia na grade da Agenda por causa da duração curta ou do horário de trabalho da profissional começar mais tarde — checado via MCP Supabase direto em produção (`ufevcgkuaagkfvyanfrd`) e via leitura de `AdminAgenda.jsx`/`ProfissionalAgenda.jsx`: `anchoredToSlot`/`apptTop`/`apptHeight` já têm um piso de altura mínima (`Math.max(cellH/2-4, ...)`, ~28px) que garante visibilidade de qualquer duração, e `offHours` (fora do expediente) só estiliza o fundo de slots **vazios** — nunca esconde um bloco já ocupado. Nenhum bug encontrado nesse caminho. **Causa real, achada pelo próprio usuário ao comparar as duas telas:** `AdminCaixa.jsx` e `ProfissionalComandas.jsx` tinham a mesma função `formatDate(iso)` fazendo `new Date(iso)` seguido de `.getDate()`/`.getMonth()` locais — `Appointment.Date` é uma coluna Postgres `DATE` pura (string `"2026-09-11"`, sem horário), e o JS interpreta uma string de data sem horário como **meia-noite UTC**; no fuso de Brasília (UTC-3) isso vira `2026-09-10T21:00` local, exibindo o dia anterior. Todo o resto do projeto já evita esse padrão fatiando a string diretamente em vez de instanciar `Date` (`MeuPerfil.jsx`, `DetalhesAgendamento.jsx`, `AdminAgendamentos.jsx`, `MinhasComanadas.jsx`, `ModalFecharConta.jsx`, etc.) — só esses 2 arquivos tinham a versão baseada em `new Date()`. **Fix:** `formatDate` nos dois arquivos trocado para `const [y, m, d] = iso.slice(0, 10).split('-'); return \`${d}/${m}/${y}\``, mesmo padrão já usado em `MeuPerfil.jsx`/`MinhasComanadas.jsx` — funciona tanto para `Appointment.Date` (10 chars) quanto para timestamps completos (`Created_at`, `Expire_at`, também formatados por essa função nesses 2 arquivos), já que o `slice(0, 10)` pega só a parte de data antes de qualquer `T`. `npx vite build` limpo (1160 módulos). **Não testado na tela nesta sessão** — usuário confirmou "está correto" depois da explicação, mas sem captura de tela do Caixa pós-fix registrada aqui.
- **Fix — "sessão expirou" falso ao abrir qualquer página sem sessão (2026-08-15, ⬜ não commitado, frontend puro)** — descoberto ao testar pela primeira vez de ponta a ponta o convite de profissional (que passou a usar Resend em vez do email do Supabase, ver `Dauth_Back_end/docs/claude/decisions.md`, "Convite de profissional via Resend"). `main.jsx` faz `GET /users/perfil/me` em todo carregamento do app (checagem silenciosa de sessão + auto-redirect de quem já está logado saindo de `/`/`/login`) sem `skipSessionExpiredRedirect` — um 401 nessa chamada, resultado normal pra qualquer visitante sem sessão, disparava o interceptor genérico (`lib/api.js`): tenta refresh, falha, `logout()` + toast "sessão expirou" + redirect pra `/login`. Isso acontecia em **qualquer** página aberta a frio sem sessão (`/login`, `/register`, `/agendar`, etc.), mas só ficou visível/grave em `/auth/accept-invite`: a profissional clicava no link do convite, o formulário piscava na tela e ~2s depois (tempo do refresh falhar) já era redirecionada pra `/login`, sem conseguir preencher os dados. Reproduzido tanto em DEV (Vercel+Render) quanto em produção — não tem relação com cookie cross-site (que só afetaria DEV). **Fix:** `lib/api.js` reestruturado — a tentativa de refresh continua acontecendo sempre pra qualquer 401 (preserva o auto-refresh silencioso de sessão expirada-mas-renovável via refresh token de 7 dias), só o bloco final de logout/redirect/toast passou a respeitar `skipSessionExpiredRedirect`; `main.jsx` ganhou esse flag no `GET /users/perfil/me` do bootstrap (mesmo padrão que `AcceptInvitePage.jsx` já usava). Efeito colateral aceito: o 401 do bootstrap continua aparecendo no Network tab/console do navegador em página pública sem sessão — é esperado, não afeta o usuário, e não dá pra suprimir sem parar de fazer a checagem (perderia o auto-redirect de quem já está logado). Replicado também no projeto `Dauth-angelica` (outro cliente, mesma base de código, mesmo bug). `npx vite build` limpo. Ver `Dauth_Back_end/docs/claude/decisions.md` para o detalhamento completo (inclui o motivo de a primeira tentativa de fix — só adicionar o flag sem reestruturar o interceptor — ter sido descartada por regressão).
- **Fix — aviso não cobria o intervalo da profissional (2026-08-13, ⬜ não commitado, frontend puro)** — reportado pelo usuário: agendar "próximo do intervalo" era permitido sem aviso nenhum e sem precisar marcar urgente. **Causa:** o aviso criado em 2026-08-02 (ver entrada abaixo) usa `buildOutsideHoursWarning` → `rangeOutsideWorkingHours`, que compara só contra `Start_time`/`End_time` — **`Break_start`/`Break_end` nunca eram consultados**, então "invade o almoço" não era um caso que a função soubesse detectar. Por que aparecia só "próximo": a grade já bloqueia clique nos slots **dentro** do intervalo, então o furo só acontece quando o agendamento **começa antes** e avança sobre ele (ex: 11:30 + serviço de 1h com intervalo às 12:00). No backend a checagem de intervalo existe (`appointmentController.insert`) mas roda **só para `role === 'Usuario'`**, e `POST /appointment/batch` (o que a Agenda usa) não checa intervalo — comportamento intencional, ver `Dauth_Back_end/docs/claude/decisions.md`, "Admin/Profissional continuam podendo agendar sem checar `WorkingHours`". **Fix (decisão do usuário: avisar, não bloquear — mantém a linha já adotada 2x):** `src/lib/workingHours.js` ganhou `rangeOverlapsBreak(wh, start, end)` e `formatBreak(wh)`, e `buildOutsideHoursWarning` passou a emitir também `"{Nome} tem intervalo das 12:00–13:00 — o serviço das 11:30 às 12:30 avança sobre esse período."`. **Nenhum caller mudou** — os 4 pontos de escrita (`NovoAgendamentoDrawer` e `TransferirDrawer` das 2 agendas + `handleSaveEdit` de `DetalhesAgendamento`) já passavam o registro completo de `WorkingHours` (`breakByProf`/`fetchWorkingHours` sempre trouxeram `Break_start`/`Break_end` — é o mesmo dado que desenha o bloco "Intervalo" na grade), então todos herdaram o aviso de graça. Item fora do expediente **não** reporta também o intervalo (`else if`): o intervalo está sempre dentro do expediente, as duas frases juntas seriam redundantes. `rangeOverlapsBreak` respeita a convenção dos 3 estados do módulo (`UNKNOWN_HOURS`/`undefined` → `false`, nunca afirma nada) e trata intervalo não cadastrado (`Break_start` nulo) como "sem conflito". Validado com 12 casos de borda em script isolado (limites exatos 11:00–12:00 e 13:00–14:00 **não** avisam; englobar, começar dentro e invadir o fim avisam) + `npx vite build` limpo.
- **Sessão 2026-08-10 (⬜ não commitado) — reversão de status, tela de Mensalistas, preço por profissional, ajustes de Agenda:**
  - **Reverter `concluido→confirmado` na Agenda** — menu de contexto de um agendamento `concluido` (`AdminAgenda.jsx`/`ProfissionalAgenda.jsx`) ganhou "Voltar para Confirmado", usando o mesmo `handleStatusChange` já existente. Backend bloqueia com 409 se a comanda daquele agendamento já estiver `Paga` (toast de erro aparece normal). Ver `Dauth_Back_end/docs/claude/decisions.md`, "Sessão 2026-08-10".
  - **Excluir agendamento — só quando `Status === 'cancelado'`** — opção nova no menu de contexto ("Excluir agendamento", ícone `trash`), consumindo o `DELETE /appointment/:id` que já existia no backend sem nenhuma UI. Modal de confirmação padrão antes de excluir. Restrito a `cancelado` por decisão do usuário (evita apagar agendamento com comanda/financeiro já gerado).
  - **Tela nova `AdminMensalistas.jsx`** (`/admin/mensalistas`, nav item em "Financeiro" ao lado de Comissões) — gestão de mensalistas separada por mês, mesmo padrão de `AdminComissoes.jsx` (KPIs + period presets + toggle + histórico). KPIs: total em aberto / deste mês / atrasado (meses anteriores) / clientes pendentes. Filtro de período (Todos/Mês atual/Mês passado/Escolher mês) + busca por nome de cliente (client-side, nas duas abas — "Em aberto" e "Histórico de quitações"). Badge "Atrasado" numa coluna de largura fixa (não gruda no nome, não quebra linha). Itens de cada comanda pendente viraram grid de colunas fixas (Serviço|Data|Valor) — mesmo padrão do `ModalFecharConta.jsx`, aplicado também dentro do `ModalPagarMensalidade`. `ModalPagarMensalidade` extraído de dentro de `AdminUsuarios.jsx` pra componente compartilhado (`src/components/ui/ModalPagarMensalidade.jsx`) — reusado pelas duas telas agora. O chip "Mensalistas" em `AdminUsuarios.jsx` continua como estava (abre o `ClientePanel`) — a tela nova é adicional, não substitui o atalho. Backend: `GET /transaction/fiado-pending?month=` + `totals`/`atrasado`, `GET /transaction/fiado-settlements` novo. Ver `Dauth_Back_end/docs/claude/decisions.md` para o detalhamento completo do backend.
  - **Filtro de período em Comandas** (`AdminCaixa.jsx`/`ProfissionalComandas.jsx`) — pills "Todos/Semana/Mês/Personalizado" acima da busca por cliente, 100% client-side sobre as comandas já carregadas (`GET /tab?limit=1000`); filtra pela data do agendamento vinculado (`Appointment.Date`), com fallback pro `Created_at` da Tab quando não há Appointment direto (combo, comanda combinada).
  - **Preço por profissional — `ServiceProfessional.Price_override` reimplementado.** A feature tinha sido perdida numa sessão anterior (WIP descartado de propósito), mas a migration de banco sobreviveu — código refeito do zero. `AdminServicos.jsx`: drawer "Profissionais" de um serviço ganhou um 2º campo clique-para-editar (preço) ao lado do já existente de comissão, mesmo padrão visual. `AgendarPage.jsx`: passo 1 (escolha de serviço) mostra `"A partir de R$ X"` (`Min_price`, o menor preço entre os profissionais vinculados) em vez do preço fixo; passo 2 (escolha de profissional) mostra o preço específico de cada uma; total na confirmação usa o preço do profissional escolhido. Ver `Dauth_Back_end/docs/claude/decisions.md` para o detalhamento completo do backend.
  - **Fix — scroll da Agenda resetava ao criar agendamento** — `NovoAgendamentoDrawer` era o único fluxo ainda chamando `load()` sem o modo silencioso; isso trocava a grade inteira pelo `PageSpinner` durante o recarregamento, desmontando o container com scroll e remontando do zero no topo. Trocado para `onSaved={() => load(true)}`, mesmo padrão já usado por `TransferirDrawer`/`FolgaDrawer`. Fix nas duas agendas.
  - **Fix — setas de navegação de dia mudando de posição** — o rótulo da data (`formatHeader(date)`) não tinha largura fixa; dias com nome mais longo empurravam a seta "próximo dia"/calendário/"Hoje"/"+ Folga" pra posições diferentes a cada troca. Rótulo ganhou `w-[170px] md:w-[320px] shrink-0 text-center` (com `truncate` de segurança) — texto centralizado numa caixa de tamanho fixo, setas nunca mais se movem. Fix nas duas agendas.
- **Sessão 2026-08-05, continuação (✅ em produção) — mensagem de erro do login + cookie cross-site no DEV:**
  - **Bug — "telefone ou senha inválidos" para qualquer falha de sessão.** `LoginPage.jsx` encadeia 3 requisições (`/auth/login` → `/users/perfil/me` → permissões) e qualquer 401 cai no mesmo `catch`. O `loginErrorMessage` lia só `data.error`/`data.erro`, mas o middleware de autorização do backend responde `{ message: 'Token not provided' }` — chave não lida, então caía no fallback `if (status === 401) return 'Telefone ou senha inválidos.'`. Resultado: **cookie de sessão recusado pelo navegador aparecia como credencial errada**, indistinguível de senha errada de verdade. Corrigido com `const isLoginRequest = err.config?.url?.includes('/auth/login')` — só o 401 vindo do próprio login mantém a mensagem de credencial; os demais passaram a dizer que a sessão não foi guardada e sugerir sair da aba anônima. Foi esse fallback que mascarou o problema abaixo e custou boa parte da investigação.
  - **Cookie third-party bloqueava login no DEV (aba anônima e Safari).** `dauth-front-end.vercel.app` → `dauth-back-end.onrender.com` são **domínios diferentes**, então o cookie de sessão é third-party: aba anônima do Chrome bloqueia por padrão e o Safari bloqueia sempre (inclusive em aba normal), o `Set-Cookie` do login é descartado e o `GET /users/perfil/me` seguinte dá 401. **`SameSite=None` não resolve** — é o que faz funcionar no desktop (ver 2026-08-04), mas bloqueio de third-party cookie é política separada e mais forte. **Produção não é afetada:** `dauth.com.br` e `back.dauth.com.br` são o mesmo domínio registrável, o cookie é first-party. Fix: `vercel.json` ganhou `{ "source": "/api/:path*", "destination": "https://dauth-back-end.onrender.com/api/:path*" }` **antes** do catch-all de SPA (ordem importa — o `/(.*)` engole tudo se vier primeiro), fazendo o navegador ver mesma origem.
  - ⬜ **Pendente:** trocar `VITE_API_URL` para `/api/v1` **na dashboard da Vercel** (Settings → Environment Variables), sem tocar no `.env.production`. O arquivo é compartilhado com o build da VPS — editá-lo derrubaria a produção (o Nginx de `dauth.com.br` não tem proxy em `/api`); o Vite prioriza a variável de ambiente real sobre o arquivo, então a dashboard isola a mudança no DEV. **Sem barra no final** (`/api/v1`, nunca `/api/v1/`): `lib/api.js:19` monta a URL do refresh por interpolação de string, e barra final vira `/api/v1//auth/refresh`, que o Express não casa — quebraria só o refresh silencioso, difícil de rastrear. Enquanto a variável não mudar, o rewrite fica **inerte** e nada muda em lugar nenhum (estado intermediário seguro, de propósito).
  - Backend na mesma sessão: correção do rate limit que bloqueava usuário legítimo no primeiro login (`trust proxy` ausente fazia o balde ser global). Ver `Dauth_Back_end/docs/claude/decisions.md`, "Sessão 2026-08-05 (continuação)".
- **Sessão 2026-08-05 (⬜ não commitado) — 5 itens:**
  - **Profissional não aparecia ao "+ Adicionar serviço" na edição** — `DetalhesAgendamento.jsx`, `TransferirDrawer` (`AdminAgenda.jsx`/`ProfissionalAgenda.jsx`): o `useEffect` que popula o select de "Profissional" só buscava opções quando o item já tinha `serviceId` — um item novo (sem serviço ainda) nunca disparava a busca, mesmo herdando `professionalId` do item anterior. Fix: `else` no mesmo efeito busca a lista geral (`GET /users?Role=Profissional` + `Role=Admin`) quando não há serviço ainda. As 3 telas de edição mantidas idênticas entre si (pedido do usuário).
  - **Texto errado de senha padrão em "Trocar senha"** — `TrocarSenha.jsx` dizia `12345678` (senha real só do fluxo `register-admin`); ajustado pra `123456789` (senha real do fluxo `reset-password`, mais comum). Tela é genérica pros 2 fluxos com mensagem estática — segue incorreta especificamente pra quem chega via `register-admin` (não resolvido, exigiria saber por qual fluxo o usuário chegou).
  - **Profissional também é cliente do salão** — `searchClients.js` (usado pelos 8 dropdowns de "Cliente") passou a buscar `Usuario`+`Profissional`+`Admin` em paralelo, não só `Usuario` — resolve não achar profissionais ao criar agendamento. Rotas `/cliente/*` (`router/index.jsx`) liberadas pra role `Profissional`; link novo em `navItemsByRole.Profissional` ("Como cliente" → "Meus atendimentos" → `/cliente`). **Achado no processo:** `MinhasComanadas.jsx` usava `GET /tab` sem parâmetro, que se auto-filtra pelo role de quem chama — pra `Profissional` filtra pelas comandas que ela **atendeu**, não as que **comprou** (mesmo endpoint, dois significados diferentes por role, ver fix de 2026-07-28 abaixo); trocado pra `GET /tab/client/${user.id}` quando o role é `Profissional`, sem essa ambiguidade. As outras 3 telas de cliente já usavam endpoints explícitos por `clientId`, sem o problema. Botão "Voltar ao painel" (`→ /profissional`, visível só pra `Profissional`) adicionado nas 4 sidebars locais de cliente.
  - **Editar preço de item no Caixa → Comandas** — paridade com o `ModalFecharConta.jsx` da Agenda (que já deixava editar `Unit_price` clicando no valor de um item). `AdminCaixa.jsx` (`TabComandas`) e `ProfissionalComandas.jsx`: painel de pagamento individual ganhou uma seção "Itens" (busca `GET /tab/:id` ao selecionar a comanda) com cada valor clicável quando `Status === 'Em aberto'` — clique vira input inline, salva via `PATCH /tab/:id/items/:itemId`, atualiza local sem rebuscar a lista inteira. Backend: `tabModels.findById` ampliado pra trazer `Tab_items` completo (antes só `Appointment`) — mudança aditiva no `select`.
  - **Busca de serviço no `/agendar` + fix "Email já cadastrado" ao criar cliente com telefone repetido** — `AgendarPage.jsx` (passo 1) ganhou input de busca por nome de serviço (`serviceSearch`), 100% client-side sobre a lista já carregada (`GET /public/services?has_professionals=true&limit=100` — teto testado e confirmado suficiente pelo usuário, 22 serviços carregados sem corte). **Fix de backend:** `authController.registerByAdmin` (usado por "Novo usuário" em `AdminUsuarios.jsx` e pelo "+" de criar cliente em `AdminAgenda.jsx`/`ProfissionalAgenda.jsx`) só tratava telefone duplicado vindo do Postgres (`23505`) — como o email é sintético (derivado do telefone), telefone repetido colide primeiro no Supabase Auth (`email_exists`/`user_already_exists`), sem tratamento, caindo no mapa genérico de erros e virando "Email já cadastrado." (campo que o usuário nunca preencheu — o cadastro é 100% por telefone). Adicionados os 2 códigos ao `catch`, devolvendo "Telefone já cadastrado." — mesmo tratamento que `authController.register` já tinha. **Nenhuma mudança de frontend necessária:** as 3 telas já checavam `msg.includes('Telefone')` pra decidir entre erro inline no campo (`setFormErrors({ phone: ... })`, sem toast) ou toast genérico (`addToast`) — só recebiam a string errada do backend. Efeito colateral notado pelo usuário: antes o bug fazia esse erro cair no toast (por não bater no `.includes('Telefone')`); corrigido, agora aparece como erro inline vermelho embaixo do campo Telefone, sem toast — comportamento correto e já esperado pelo código, só nunca tinha sido acionado do jeito certo. Ver `Dauth_Back_end/docs/claude/decisions.md`, "Sessão 2026-08-05", para o detalhamento completo dos 5 itens.
- **Remover 1 item do pagamento sem fechar a comanda + pagamento individual via RPC (2026-08-03, ⬜ não commitado)** — o X de uma linha no `ModalFecharConta` riscava todos os itens do mesmo `Booking_group` (o toggle sempre agiu por Tab, e num atendimento combinado todos os serviços vivem na MESMA Tab). Agora o X é por item e os itens excluídos vão em `excluded_item_ids` no `POST /tab/batch-pay` — ficam pendentes no banco e seguram a comanda em aberto. Junto: as telas que exibem valor de comanda em aberto passaram a mostrar só o que resta a cobrar, e `handlePagar` (`AdminCaixa.jsx`/`ProfissionalComandas.jsx`) trocou `POST /transaction` + `PATCH /tab/:id` por `POST /tab/batch-pay` com `tab_ids: [id]` — o caminho antigo não tocava em `Tab_items` e perdia a comissão por profissional em comanda multi-item. **Exige a migration `batch_pay_tabs_item_exclusion.sql` aplicada antes do deploy do backend.** Ver `Dauth_Back_end/docs/claude/decisions.md`, "Remover 1 item do pagamento sem fechar a comanda".
- **Fix — agendamentos da manhã sumiam da grade da Agenda (2026-08-03, ⬜ não commitado, frontend puro)** — usuário reportou que, num dia cheio em produção (08/08/2026), nenhum agendamento aparecia no período da manhã, embora existissem. Confirmado direto no banco de produção: 25 registros naquele dia, sendo 5 pendentes antes das 11h (09:00 com Candida/kreicy/daiane, 10:00 com Candida/kreicy) — exatamente os que não apareciam. **Causa:** `AdminAgenda.jsx` chamava `GET /appointment?date=` **sem `limit`**, então o backend aplicava o default de `parsePagination` (**20**); e `appointmentModels.find` ordena `.order('Date', desc).order('Start_time', desc)` — ordem decrescente por horário + corte em 20 descarta justamente os primeiros horários do dia. O `.order()` decrescente foi adicionado em 2026-07-26 para as listas de histórico (`AdminAgendamentos`, ver entrada daquela data); a grade da Agenda herdou junto, e o que antes era ordem arbitrária do banco (sintoma intermitente) virou "some sempre a manhã". **Fix:** `limit: 50` explícito nas duas agendas — `AdminAgenda.jsx` (`GET /appointment`) e `ProfissionalAgenda.jsx` (`GET /appointment/my`, que tem o mesmo `.order()` desc + `applyPagination` em `findByProfessional`). Valor escolhido pelo usuário ("mais de 50 por dia não vamos ter"); nenhuma mudança de backend. **Teto continua existindo:** na tela do Admin os 50 são o dia inteiro somando **todas** as profissionais (08/08 tinha 25 com 5 profissionais), então mais profissionais ou dias mais cheios trazem o truncamento de volta — silenciosamente e sempre pela manhã, sem nenhum aviso na tela. Na `ProfissionalAgenda` é por profissional só, bem mais folgado. Eliminar a classe do bug exigiria ordenar ascendente na grade ou paginar até esgotar — não feito, decisão de subir o teto.
- **Horário de trabalho visível na Agenda + aviso ao agendar fora dele (2026-08-02, ⬜ não commitado, frontend puro)** — pedido do usuário: até aqui o `WorkingHours` de cada profissional só tinha efeito no fluxo público `/agendar` (o `GET /public/availability` já filtrava slots por ele); na Agenda do Admin/Profissional o horário era invisível e nada avisava ao encaixar um atendimento fora do expediente. **Nenhuma mudança de backend** — o dado já estava disponível: `AdminAgenda.jsx` já buscava `GET /working-hours/professional/:id` por profissional a cada troca de data (state `breakByProf`, populado num `useEffect` próprio), mas só consumia `Break_start`/`Break_end` para desenhar o bloco "Intervalo", ignorando `Start_time`/`End_time`; `ProfissionalAgenda.jsx` idem, com o state `workingHour` (1 profissional só).
  - **Helper novo `src/lib/workingHours.js`** (compartilhado pelas duas agendas — exceção consciente à duplicação de helpers que existe entre `AdminAgenda`/`ProfissionalAgenda`, porque a mesma lógica seria repetida em 4 pontos): `outsideWorkingHours(wh, slot)`, `rangeOutsideWorkingHours(wh, start, end)`, `formatWorkingHours(wh)` (→ `"09:00–18:00"` ou `null`), `workingHoursLabel(wh, loaded)` (texto do cabeçalho), `fetchWorkingHours(ids, weekday)` (→ `{ [id]: wh | null | UNKNOWN_HOURS }`, nunca rejeita) e `buildOutsideHoursWarning(items, whByProf)` (→ string do aviso ou `null`).
  - **Convenção do módulo — 3 estados, não 2:** `wh` objeto = trabalha no intervalo; `wh === null` = **não trabalha naquele dia da semana** (nenhum registro em `WorkingHours` para aquele `Weekday`), e por isso `outsideWorkingHours(null, slot)` é `true`; `wh === UNKNOWN_HOURS` (constante exportada, string `'unknown'`) = **não deu pra saber**, devolvido pelo `.catch` de `fetchWorkingHours`. `undefined` (profissional ausente do mapa, ex: busca ainda em curso) é tratado igual a `UNKNOWN_HOURS`. Nos dois casos desconhecidos, `outsideWorkingHours`/`rangeOutsideWorkingHours` retornam `false` — nada é listrado e nenhum aviso é emitido. **Isso é o ponto do desenho:** na primeira versão o `.catch` devolvia `null`, então uma falha de rede era indistinguível de "não cadastrou horário" e a tela afirmava com confiança *"Fulana não trabalha hoje"* para todas as colunas de uma vez. Erro nunca deve virar afirmação sobre a agenda de alguém.
  - **Texto do cabeçalho centralizado em `workingHoursLabel(wh, loaded)`** — antes cada tela montava o seu (`formatWorkingHours(...) ?? 'Não trabalha hoje'` no Admin, `?? 'Sem horário cadastrado para hoje'` no Profissional: mesma condição, frases diferentes). Retorna `""` quando `loaded === false`, `"Horário indisponível"` para `UNKNOWN_HOURS`, `"Não trabalha hoje"` para `null` e `"Trabalha das 09:00 às 18:00"` (frase completa, não só o intervalo) quando há horário. Renderizado em `text-[11px]` (não mais `font-mono text-[10px]`), com `text-danger` quando é `null` para o caso "não trabalha" saltar aos olhos, e `text-ink-3` nos demais.
  - **Visual:** slot fora do expediente ganha a classe `.bg-off-hours` (listrado diagonal sutil, regra nova em `index.css` — não é utilitário Tailwind) + `title="Fora do horário de trabalho"`. **Continua clicável** — abrir o drawer de novo agendamento ali é permitido de propósito; a marcação é informativa. Precedência de fundo na célula: `past || onBreak || onLeave` (→ `bg-surface-2`) vence sobre `offHours`. O cabeçalho de cada coluna de profissional (grade desktop, seletor mobile do Admin, coluna única do Profissional) mostra o expediente do dia embaixo do nome via `workingHoursLabel` (ver acima), e a legenda no rodapé ganhou a entrada "Fora do expediente".
  - **Guard `whLoaded` — state nas duas telas:** enquanto o fetch de horários não termina, o estado ainda guarda o horário do **dia anterior** (ou nada, no primeiro render); sem o guard a grade piscaria listrada no mount e, ao navegar entre dias, mostraria o expediente do dia errado por um instante. Ambas as telas usam um state `whLoaded` setado para `false` antes do fetch e `true` no `.finally`. **A primeira versão do Admin derivava isso de `Object.keys(breakByProf).length > 0`** — o que cobria só o mount, nunca a troca de data (o mapa antigo continua não-vazio), e por isso foi trocado por state de verdade. O texto do cabeçalho também fica vazio até carregar (`workingHoursLabel(wh, false) === ''`).
  - **Slot parcialmente fora do expediente não é marcado** — `outsideWorkingHours` compara só o **início** do slot, então com expediente até 17:45 o slot 17:30–18:00 não fica listrado. Escolha deliberada: marcar levaria o último slot de praticamente todo expediente a ficar listrado, ruído visual constante para uma precisão de 15 min que ninguém consulta na grade.
  - **Aviso ao salvar (não bloqueio):** `NovoAgendamentoDrawer` e `TransferirDrawer` nas duas agendas, mais `handleSaveEdit` de `DetalhesAgendamento.jsx`, ganharam `handleSalvar(skipOffHoursCheck = false)` — antes de enviar, monta o aviso com `buildOutsideHoursWarning` e, se houver, abre um `Modal` de confirmação em vez de salvar; confirmar chama `handleSalvar(true)` e segue o fluxo normal. Mensagem por item: `"{Nome} atende das 09:00–18:00 — o serviço das 19:00 às 20:00 está fora desse horário."` ou `"{Nome} não trabalha neste dia — o serviço está marcado para 14:00."`, seguidas de `"Deseja agendar mesmo assim?"`. No `ProfissionalAgenda` o nome vira "Você" (é sempre o próprio profissional logado).
  - **De onde vem o `wh` em cada fluxo:** o **drawer de criação** recebe o mapa pronto por prop (`whByProf={breakByProf}` no Admin, `workingHour={workingHour}` no Profissional) — a data é fixa (a data da grade), então não há fetch novo. Os **3 fluxos de edição** (`TransferirDrawer` nas 2 agendas + `handleSaveEdit` de `DetalhesAgendamento`) buscam na hora de salvar via `fetchWorkingHours`, porque **editar pode mover o agendamento pra outra data**, e aí o dia da semana muda e com ele o expediente — checar contra o horário da data antiga daria o aviso errado. O weekday sai do campo de data editado (`newDate` nos drawers, `editDate` em DetalhesAgendamento), o mesmo valor que vai no PATCH. Erro nessa busca é engolido (`catch {}`) e o salvamento prossegue — é um aviso, nunca pode travar a edição. Os botões usam `loading={saving || checkingHours}`.
  - **`DetalhesAgendamento` — só Admin/Profissional chegam nesse aviso.** A revisão desta feature revelou que a tela (acessível a todos os roles, e para a qual `MeusAgendamentos.jsx` do cliente navega direto) exibia o botão "Editar" gated só por `canReschedule` (`Status ∈ {pendente, confirmado}`), **sem checagem de role** — o cliente movia o próprio agendamento pra qualquer data/horário, contornando o filtro de `WorkingHours` que o `/agendar` aplica. Por decisão da admin (*"a edição de data/hora deve ser apenas por profissional e admin"*), o botão passou a exigir `canEdit &&` e a barra de ações que o envolvia (`{(canEdit || canReschedule) && ...}`, cujo `canReschedule` existia só pra mostrar esse botão ao cliente) virou `{canEdit && ...}` — senão renderizaria uma div vazia pro cliente. **Backend acompanhou** (gate de UI não fecha a API): `appointmentController.update` passou a rejeitar `Service`/`Date`/`Start_time`/`End_time`/`Add_services`/`Update_services` para `Usuario` com 403. Isso reverte parcialmente a decisão de 2026-07-12 que tinha aberto a edição para o Cliente — ver `Dauth_Back_end/docs/claude/decisions.md`, "Edição de data/hora restrita a Admin/Profissional". O nome da profissional na mensagem do aviso vem de `itemMeta[i].professionalOptions` quando o select está carregado, com fallback para `item.Professional`.
  - **⚠️ Armadilha repetida do `onClick={handleSalvar}`:** o React passa o `SyntheticEvent` como 1º argumento, que cairia em `skipOffHoursCheck` (objeto = truthy) e pularia o aviso em 100% dos cliques — exatamente o bug já cometido e corrigido em `AgendarPage.jsx` (2026-07-26, ver entrada abaixo). Os 4 botões usam `onClick={() => handleSalvar()}`, com comentário no código. **Qualquer handler novo com parâmetro de "pular checagem" neste projeto precisa da arrow function.**
  - **`Modal.jsx`:** a `<p>` da mensagem ganhou `whitespace-pre-line` — o aviso quebra linha quando há mais de um serviço fora do horário. Mudança aditiva, não afeta as mensagens de uma linha já existentes.
  - **Decisão de avisar em vez de bloquear** é a mesma já registrada em `Dauth_Back_end/docs/claude/decisions.md` (2026-07-26, "Admin/Profissional continuam podendo agendar sem checar `WorkingHours`") — o backend continua aceitando qualquer horário, nenhuma validação nova foi adicionada lá. Um cliente que chame a API direto não vê aviso nenhum, por definição.
  - **Cobertura:** os 4 pontos de escrita de agendamento na área logada têm o aviso — `NovoAgendamentoDrawer` e `TransferirDrawer` das 2 agendas + `handleSaveEdit` de `DetalhesAgendamento`. O fluxo público `/agendar` não precisa: `GET /public/availability` já filtra os slots por `WorkingHours` antes de oferecê-los. Um cliente que chame a API direto não vê aviso nenhum, por definição — nenhuma validação foi adicionada no backend.
  - **Não validado em tela** por sessão nenhuma até aqui: só `npx vite build` limpo. Vale abrir a Agenda num dia com profissional de horário parcial para conferir o listrado e disparar o modal.
- **Profissional por serviço na edição de agendamento + fix de bloco com buraco na Agenda (2026-08-01, ✅ commitado, em produção)** — pedido do usuário: poder trocar a profissional de um agendamento na edição (fase 1) e, dentro do mesmo atendimento com N serviços, atribuir uma profissional diferente a **cada** serviço individualmente (fase 2 — "3 serviços, 1 precisa de outra profissional pra auxiliar no tempo"). **Fase 1:** select de "Profissional" adicionado em `DetalhesAgendamento.jsx` e no `TransferirDrawer` (`AdminAgenda.jsx`/`ProfissionalAgenda.jsx`), populado via `GET /service/:id/professionals` do serviço já selecionado — só lista quem já atende aquele serviço; troca não revalida disponibilidade de horário na hora (decisão do usuário — o backend já revalida `checkConflict` sozinho no PATCH quando `Professional` muda). Aproveitado pra corrigir a mesma dívida já documentada abaixo (2026-07-30) no `TransferirDrawer`, que só decompunha o 1º serviço do bloco. **Fase 2:** cada item da edição ganhou seu próprio campo `professionalId` (antes era 1 só pro bloco inteiro) — item que muda de profissional sai do bloco fundido e vira um `Appointment` próprio, ligado ao original pelo mesmo `Booking_group` (mesmo mecanismo do atendimento combinado multi-profissional). Novos campos de `PATCH /appointment/:id` no backend: `Remove_services` (array de `Item_id`, remove da `Appointment_services` e recalcula `Start_time`/`End_time` pelos restantes) e `Booking_group` (grava a tag direto no PATCH). `handleSaveEdit`/`handleSalvar` separam os itens em `sameProf` (continuam fundidos — `Add_services`/`Update_services`) e `diffProf` (saem via `Remove_services` se já existiam, depois um `POST /appointment` por item cria o `Appointment` separado, sequencial sem atomicidade — mesmo risco já aceito em outros fluxos multi-item do projeto). **Fix de bloco com buraco (mesma sessão, reportado ao ver o resultado na tela):** depois de mover um item pra outra profissional, o `Appointment` original ainda cobre o intervalo completo (`Start_time`/`End_time` recalculados como min/max dos itens que sobraram) — a Agenda desenhava isso como 1 retângulo contínuo mostrando "tempo ocioso" que já era de outra profissional, e bloqueava o slot vazio pra novo agendamento. `splitIntoSegments(appt)` (novo helper, duplicado em `AdminAgenda.jsx`/`ProfissionalAgenda.jsx`) quebra o bloco em N segmentos visuais contíguos a partir dos horários reais de `Services[]`; `visualAppointments` (`useMemo`) substitui `appointments` em tudo que desenha/posiciona blocos (`anchoredToSlot`, `coversSlot`/`occupied`, `computeColumns`, agora keyado por `_segKey` em vez de `UUID` pra não colidir entre segmentos do mesmo Appointment); cliques/context menu sempre usam `a._original ?? a` (o Appointment completo, não o segmento truncado) pra abrir a edição corretamente. Sem `Services[]` com buraco, nada muda — 1 segmento só, igual antes. **Limitação aceita:** 2+ itens movidos pra uma MESMA nova profissional na mesma edição viram Appointments separados dela, não se fundem entre si. Ver `Dauth_Back_end/docs/claude/decisions.md`, "Profissional por serviço na edição de agendamento".
- **Fix — edição de agendamento com múltiplos serviços: só mostrava 1 serviço, depois selects vazios, depois não salvava (2026-07-30, ⬜ backend não commitado)** — 3 bugs em cadeia em `DetalhesAgendamento.jsx` (`openEdit`/`handleSaveEdit`), todos no fluxo de "Editar" um agendamento com serviços fundidos (`item.Services[]`, ver `Appointment_services` no backend): (1) `openEdit` só decompunha `item.Service_id` (sempre o 1º serviço) em vez de `item.Services` inteiro — corrigido pra mapear todo o array. (2) O fix de (1) declarou uma `const services` local que sombreava o **estado** `services` (opções do dropdown) — a checagem que dispara `GET /service?professional=...` passou a ler o array errado (sempre não-vazio) e nunca buscava as opções, deixando os `SearchableSelect` sem nada pra casar com o valor selecionado (apareciam vazios). Corrigido renomeando a variável local para `itemServices`. (3) Mesmo exibindo e selecionando certo, salvar não persistia serviços 2+ — não existia nenhum caminho no backend pra **atualizar** um item já existente de `Appointment_services` (só inserir via `Add_services`). Endpoint `PATCH /appointment/:id` ganhou `Update_services: [{ Id, Service, Start_time, End_time }]` (backend, ver `Dauth_Back_end/docs/claude/decisions.md`); `formatAppointment` passou a expor `Item_id` por serviço (o UUID da linha `Appointment_services`, usado como `Id`). `openEdit` agora guarda `itemId`/`isNew` por item; `handleSaveEdit` separa itens novos (`Add_services`) dos já existentes editados (`Update_services`) e manda os dois no mesmo `PATCH`; botão "x" de remover só aparece pra itens `isNew` (pré-existentes não têm endpoint de remoção ainda). **Fix lateral no backend:** editar o 1º serviço (via `Service`/`Start_time`/`End_time` do PATCH principal, não `Update_services`) atualizava só a coluna do `Appointment`, nunca a linha correspondente em `Appointment_services` — corrigido com `syncFirstServiceItem`, sem mudança de frontend necessária. Teste novo `tests/appointmentEditServices.test.js` (6 casos). **Fora do escopo:** `TransferirDrawer` (`AdminAgenda.jsx`/`ProfissionalAgenda.jsx`) tem o mesmo padrão antigo (só decompõe o 1º serviço) — não foi tocado, candidato a receber o mesmo tratamento se o mesmo sintoma aparecer ali.
- **Fix — painel de comanda/pedido selecionado "sumia" no fim da página em mobile (2026-07-26)** — reportado pelo usuário: em `AdminCaixa.jsx` (abas Comandas e Produtos) e `ProfissionalComandas.jsx` (mesmas 2 sub-abas), o layout usa `grid grid-cols-1 lg:grid-cols-[1fr_400px]` — lista + painel lado a lado só a partir de `lg`; abaixo disso (mobile/tablet, a maioria dos acessos reais) vira uma coluna só, então o painel "Comanda selecionada"/"Pedido selecionado" só aparecia **depois** de toda a lista no fluxo do documento, exigindo rolar a tela inteira pra ver a situação de algo que acabou de ser clicado. **Fix (4 blocos idênticos nos 2 arquivos):** quando há um item selecionado, a lista ganha `hidden lg:block` (some em mobile, continua visível em desktop) e o painel ganha um botão "Voltar para a lista" (`lg:hidden`, ícone `arrowLeft`) que limpa a seleção — vira um padrão mestre-detalhe: em mobile, selecionar um item troca a lista pelo painel em tela cheia; em desktop nada muda (os dois sempre lado a lado, como já era). Nenhuma mudança de dado/API, só CSS condicional e 1 botão novo por bloco.
