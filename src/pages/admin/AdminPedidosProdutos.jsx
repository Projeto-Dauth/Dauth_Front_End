...
export default function AdminPedidosProdutos() {
  ...
  // Produto selecionado no formulário (para exibir preço unitário estimado)
  const selectedProduct = products.find((p) => p.UUID === orderForm.Product_id)
  const estimatedTotal = selectedProduct
    ? formatPrice(selectedProduct.Price * Number(orderForm.Quantity || 1))
    : null
    ...
    <AppLayout sidebar={sidebar}>
      ...
      {loading ? <PageSpinner /> : orders.length === 0 ? (
        <EmptyState
          ...
          action={filterStatus ? undefined : openNewOrder}
        ...
        <>
          ...
          <div className="hidden md:block bg-surface border border-line rounded-lg overflow-hidden">
            <table className="w-full border-collapse">
              <thead>
                <tr>
                  {['Data', 'Cliente', 'Produto', 'Qtd', 'Total', 'Método', 'Vendido por', 'Status', ''].map((h) => (
              ...
              <tbody>
                {orders.map((o) => (
                  <tr key={o.UUID} className="hover:bg-surface-2 transition-colors">
                    ...
                    <td className="px-3.5 py-3 text-[13px] text-ink-2 border-b border-line-2">{o.SoldBy?.Name ?? '—'}</td>
                    ...
                    <td className="px-3.5 py-3 text-right border-b border-line-2">
                      <div className="flex gap-1 justify-end">
                        <Button variant="ghost" size="sm" onClick={() => setDetailDrawer(o)}>
          ...
          <div className="w-full md:w-[420px] bg-surface border-l border-line h-full overflow-y-auto p-5 md:p-7 flex flex-col">
            ...
            <form onSubmit={handleSaveOrder} className="flex flex-col gap-4 flex-1">
              ...
              <div className="flex gap-2 mt-auto pt-4">
                <Button type="button" variant="ghost" className="flex-1 justify-center" onClick={() => setNewDrawer(false)}>
                  Cancelar
                </Button>
            ...
            {detailDrawer.Status === 'encomendado' && (
              <div className="flex flex-col gap-2 mt-auto pt-4 border-t border-line">
                <Button
                  ...
                  onClick={() => setPayModal(detailDrawer)}
                >
                ...
                <Button
                  ...
                  onClick={() => setCancelModal(detailDrawer)}
                >
                  Cancelar pedido
                </Button>
              </div>
